...
from riffy.exceptions import InvalidChunkError, MissingChunkError
...
class TestReplaceChunk:
    ...
    def test_replace_nonexistent_chunk(self, valid_pcm_wav):
        ...
        with pytest.raises(MissingChunkError, match="Chunk 'INFO' not found"):
...
class TestAddChunk:
    ...
    def test_add_chunk_with_invalid_length(self, valid_pcm_wav):
        ...
        with pytest.raises(InvalidChunkError, match="must be exactly 4 characters"):
...
class TestCopyChunkFromParser:
    ...
    def test_copy_nonexistent_chunk(self, valid_pcm_wav, valid_mono_wav):
        ...
        with pytest.raises(MissingChunkError, match="Chunk 'INFO' not found in source"):
            parser2.copy_chunk_from_parser("INFO", parser1)
...
class TestWriteWav:
    ...
    def test_write_wav_missing_fmt_chunk(self, valid_pcm_wav, tmp_path):
        ...
        output_path = tmp_path / "output.wav"
        with pytest.raises(MissingChunkError, match="without 'fmt ' chunk"):
            parser.write_wav(output_path)
    ...
    def test_write_wav_missing_data_chunk(self, valid_pcm_wav, tmp_path):
        ...
        output_path = tmp_path / "output.wav"
        with pytest.raises(MissingChunkError, match="without 'data' chunk"):
            parser.write_wav(output_path)
