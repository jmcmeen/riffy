...
class TestFullParsingWorkflow:
    ...
    def test_reparse_discards_modifications(self, valid_pcm_wav):
        """Re-parsing re-reads from disk and discards in-memory edits."""
        parser = WAVParser(valid_pcm_wav["filepath"])

        parser.add_chunk("TEST", b"temporary")
        assert "TEST" in parser.chunks

        parser.parse()  # re-read from disk
        assert "TEST" not in parser.chunks
...
class TestEdgeCases:
    ...
    def test_very_short_audio(self, temp_wav_dir):
        ...
        create_wav_file(filepath, num_samples=1)
    ...
    def test_odd_chunk_size_padding(self, temp_wav_dir):
        ...
        create_wav_file(filepath, extra_fmt_data=b"\x00")
    ...
    def test_various_sample_rates(self, temp_wav_dir):
        ...
        for rate in sample_rates:
            ...
            create_wav_file(filepath, sample_rate=rate, num_samples=rate)
    ...
    def test_various_bit_depths(self, temp_wav_dir):
        ...
        for bits in bit_depths:
            ...
            create_wav_file(filepath, bits_per_sample=bits, num_samples=1000)
    ...
    def test_max_channels(self, temp_wav_dir):
        ...
        for channels in [1, 2, 4, 6, 8]:
            ...
            create_wav_file(filepath, channels=channels, num_samples=1000)
